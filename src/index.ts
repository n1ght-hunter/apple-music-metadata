...
export async function findJSONLD(
    ...
    album: boolean = false,
    fast: boolean = false,
): Promise<RawApplePlaylist | HalfSong | HalfPlaylist | undefined> {
        ...
        if (data["@type"] === "MusicPlaylist") {
            ...
            return {
                ...
                tracks: (await Promise.all(
                    track.map(
                        async (songData: any) =>
                            await getSong(songData.url, true)
                    )
                )),
            };
        }
...
export async function getSong(
    ...
    const { artist, album } = (await findJSONLD(document)) as HalfSong;
...
export async function getPlaylist(
    ...
    try {
        ...
        const { type, name, author } = (await findJSONLD(document, true, true)) as HalfPlaylist;
...
export async function autoGetApple(
    ...
    if (
        RegExp(/https?:\/\/music\.apple\.com\/.+?\/album\/.+?\/.+?\?i=/).test(
            url
        )
    ) {
